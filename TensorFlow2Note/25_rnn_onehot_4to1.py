...
"""
@Filename: 25_rnn_onehot_4to1.py
@Function: TensroFlow2 API tf.keras 实现 RNN 对字母进行预测
@Python Version: 3.8
@Author: Wei Li
@Date：2021-06
@Usage: $ python 25_rnn_onehot_4to1.py 
"""
...
# ----------------------------------------------------------
input_word = "abcde"
word_to_id = {'a': 0, 'b': 1, 'c': 2, 'd': 3, 'e': 4}  # 单词映射到数值 id 的词典
...
x_train = [
    [id_to_onehot[word_to_id['a']], 
     id_to_onehot[word_to_id['b']], 
     id_to_onehot[word_to_id['c']], 
     id_to_onehot[word_to_id['d']]],
    [id_to_onehot[word_to_id['b']], 
     id_to_onehot[word_to_id['c']], 
     id_to_onehot[word_to_id['d']], 
...
# 表示为独热码有 5 个输入特征，每个时间步输入特征个数为 5
...
# 断点续练模型
# ----------------------------------------------------------
checkpoint_save_path = r"./checkpoint/rnn_onehot_4to1/rnn_onehot_4to1.ckpt"
...
weight_txt = os.path.join(csv_file, "rnn_onehot_4to1.txt")
...
for i in range(preNum):
    ...
    # 使 alphabet 符合 SimpleRNN 输入要求：[送入样本数， 循环核时间展开步数， 每个时间步输入特征个数]
    # 此处验证效果送入了 1 个样本，送入样本数为 1；
    # 输入 1 个字母出结果，所以循环核时间展开步数为 1; 
    # 表示为独热码有 5 个输入特征，每个时间步输入特征个数为 5
